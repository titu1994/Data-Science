...
def plot_cities():
    ...
    for language, (x, y) in plots.items():
        sns.plt.scatter(x, y, color=colors[language], marker=markers[language],
                          label=language, zorder=10)

    sns.plt.legend(loc=0)
    sns.plt.axis([-130,-60,20,55])
    sns.plt.title("Favorite Programming Languages in the US")
...
def classify_and_plot_grid(k=1):
    ...
    for longitude in range(-130, -60):
        for latitude in range(20, 55):
            predicted_language = knnClassify(k, cities, [longitude, latitude])
    ...
    sns.plt.legend(loc=0)
    sns.plt.axis([-130,-60,20,55])
    sns.plt.title(str(k) + "-Nearest Neighbor Programming Languages")
